...
# VERSION: 20260924.45 (修复 APK 更新模式恢复 PassWall 源)
#==============================================
...
find_apk_url() {
  local pkg="$1" ver="$2" feed url feeds
  [ "$SF_OK" = "1" ] && [ -n "$SF_BASE" ] || return
  [ -n "$ver" ] || ver=$(get_repo_version "$pkg")
  [ -n "$ver" ] || return
  # LuCI 主包在 passwall_luci，sing-box/hysteria 等运行核心在
  # passwall_packages；按包类型优先探测正确 feed，避免先报无意义的 404。
  case "$pkg" in
    luci-*|luci_*) feeds="passwall_luci passwall_packages passwall2" ;;
    *) feeds="passwall_packages passwall_luci passwall2" ;;
  esac
...
apk_install() {
  ...
  if [ "$PKG_MGR" != "apk" ]; then
    ...
    local total=0 cur=0 dep deps dep_rc failed_deps=""
    if [ "$installed_main" = "1" ]; then
      deps=""
    else
